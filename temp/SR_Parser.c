...
int isStackEmpty() { return stack.top == -1; }
void push(char symbol) { stack.items[++stack.top] = symbol; stack.items[stack.top + 1] = '\0'; }
char pop() { return isStackEmpty() ? '\0' : stack.items[stack.top--]; }
char peek() { return isStackEmpty() ? '\0' : stack.items[stack.top]; }
...
void parse() {
    ...
    while (checkAndReduce(action)) printState(action);
